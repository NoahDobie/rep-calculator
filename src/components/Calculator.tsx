...
function PercentageDisplay({ percentage, oneRepMax, unit, isDarkMode }: PercentageDisplayProps) {
    return (
        <div className={`transition flex justify-between border rounded-md p-2 text-sm sm:text-sm md:text-md lg:text-lg xl:text-xl ${isDarkMode ? 'border-dark-border' : 'border-light-border'} hover:border-red-900 focus:border-red-900`} >
...
function Calculator({ weight, reps, unit, isDarkMode }: CalculatorProps) {
  ...
  return (
    <div className={`transition-colors duration-300 p-4 rounded-lg shadow-md mt-3 ${isDarkMode ? 'bg-dark-secondary text-dark-text' : 'bg-light-secondary text-light-text'}`}>
